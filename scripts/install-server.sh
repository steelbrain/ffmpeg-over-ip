#!/bin/sh
# install-server.sh — Install the ffmpeg-over-ip server in the current directory.
#
# Usage:
...
# output already exists. Set FOIP_FORCE=1 to re-download.
...
for bin in "$BINARY" ffmpeg ffprobe; do
    if [ ! -e "$bin" ]; then
        echo "Expected $bin in current directory but it's missing." >&2
...
# Server zip ships with the server binary plus ffmpeg and ffprobe.
...
if [ -e "$CONFIG" ]; then
    ...
    listen_ip=$(prompt "Listen IP (0.0.0.0 = all interfaces)" "0.0.0.0")
    listen_port=$(prompt "Listen port" "5050")
    auth_secret=$(prompt_required "Auth secret (clients must use the same value)")

    esc_ip=$(printf '%s' "$listen_ip" | sed -e 's/\\/\\\\/g' -e 's/"/\\"/g')
    esc_port=$(printf '%s' "$listen_port" | sed -e 's/\\/\\\\/g' -e 's/"/\\"/g')
    esc_secret=$(printf '%s' "$auth_secret" | sed -e 's/\\/\\\\/g' -e 's/"/\\"/g')

    printf '{\n  "log": "stdout",\n  "address": "%s:%s",\n  "authSecret": "%s"\n}\n' \
        "$esc_ip" "$esc_port" "$esc_secret" > "$CONFIG"
    echo "Wrote $CONFIG."
fi
...
echo "  ./$BINARY"
echo "Make sure the configured port is reachable (firewall, security group, etc.)."
