# install-server.ps1 — Install the ffmpeg-over-ip server in the current directory.
#
# Usage:
#   irm https://ffmpeg-over-ip.com/install-server.ps1 | iex
#
# Re-runs are idempotent: each step (download, config) is skipped if its
# output already exists. Set $env:FOIP_FORCE='1' to re-download.
...
# Verify the extraction produced what we expect — server zip ships with the
# server binary plus ffmpeg and ffprobe.
foreach ($expected in @($Binary, 'ffmpeg.exe', 'ffprobe.exe')) {
    if (-not (Test-Path $expected)) {
        throw "Expected $expected in current directory but it's missing. The release zip may be malformed; retry with `$env:FOIP_FORCE='1'."
    }
}
...
foreach ($bin in @($Binary, 'ffmpeg.exe', 'ffprobe.exe')) {
...
if (Test-Path $Config) {
    ...
    $listenIp   = Read-Default 'Listen IP (0.0.0.0 = all interfaces)' '0.0.0.0'
    $listenPort = Read-Default 'Listen port' '5050'
    $authSecret = Read-Required 'Auth secret (clients must use the same value)'
...
Write-Host "  .\$Binary"
Write-Host 'Make sure the configured port is reachable (firewall, security group, etc.).'
